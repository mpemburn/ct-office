...
$.DDComboBox = function(input, button, options) {
  ...
  $(button).mousedown(function(event) {
    ...
    if(input.value == input.title) {//this chunk is because IE is retarded and fails to empty the text box fast enough
      input.value = "";
    }
    
    showAutocomplete();
    if($.browser.msie){
        //needs something to slow down the processing of the JS
        input.focus();
    }else{
        input.focus();//the focus when the arrow button is pressed
    }
  ...
  function showAutocomplete() {
    ...
    */
	...
	function trimWords(value) {//this function trims the words inputed into the autocomplete box
		if ( !value ) {
			return [""];
		}
    ...
    //this function scans through the list in search of the items
    //matching the autocomplete inputed value
...
$.DDComboBox.Cache = function(options) {
	...
	function populate(){
		...
		for ( var i = 0, ol = options.data.length; i < ol; i++ ) {
            ...
            var cleanedValue = rawValue[0].replace(/<\/?[^>]+(>|$)/g, "");//removes the HTML from the rawValue object
            //rawValue[0] is used because rawValue is an object containing a string, this is for IE
            
			// if the match is a string
			var row = {
				value: cleanedValue,//this is the secret list where searches are made for the auto-completion[non-visible]
				data: rawValue,//this is the data that goes into the visible list
				//result: options.formatResult && options.formatResult(rawValue) || value
				result: cleanedValue//this is the text that appears in the text box whe you click in the list
			};
		...
		load: function(q) {
			...
			if (options.matchSubset) {
				for (var i = q.length - 1; i >= options.minChars; i--) {
					...
					if (c) {
						var csub = [];
						$.each(c, function(i, x) {//parses ALL the data in search for matches, the x parameter is 1 node from data
							if (matchSubset(x.value, q)) {
								csub[csub.length] = x;
							}
						});
...
$.DDComboBox.Select = function (options, input, select, config) {
	...
	function init() {
		...
		list = $("<ul>").appendTo(element).mouseover( function(event) {
			...
			select();
            input.focus();
			//return false;//commenting this out makes clicks on links follow the link
		}).mousedown(function() {
	...
	function fillList() {//this function populates the list of matching items
		list.empty();
	...
	return {
		...
		show: function() {
            ...
            if(options.scroll) {//if scrolling is turned on in the options, this block resizes the div
                list.scrollTop(0);
                list.css({
