...
class CI_Cizacl
{
	...
	function check_cizacl()
	{
		...
		{
			//*** Modification of default behavior: Show error dialog and redirect user to their default page
			$redirect = $this->cizacl_mdl->getRoleRedirect($role);
			show_dialog($this->lang->line('no_permissions'),null,$redirect);
		}
	}
	...
	function css()
	{
		...
		foreach($array as $value)	{
			$data .= '<link href="'.base_url().'css/'.$value.'.css" rel="stylesheet" type="text/css" />'.PHP_EOL;
		}
		$data .=	'<!--[if lte IE 8]>'.PHP_EOL.
					'<link href="'.base_url().'css/cizaclIE.css" rel="stylesheet" type="text/css" />'.PHP_EOL.
					'<![endif]-->'.PHP_EOL;
		return $data;
	}
	...
	function scripts()
	{
		$array = array(
			...
			'jquery.colorbox.min',
			'i18n/jqgrid/grid.locale-'.$this->cizacl_mdl->getAbbr('js/i18n/jqgrid/','grid.locale-xx.js'),
			'jquery.jqGrid.min',
		);
		$data = '';
		foreach($array as $value)	{
			$data .= '<script type="text/javascript" src="'.base_url().'js/'.$value.'.js"></script>'.PHP_EOL;
		}
	...
	function debug_has($resource)
	{
		return $this->cizacl->has($resource);
	}

	function debug_isAllowed($role = null, $resource = null, $privilege = null)
	{
		if (!is_numeric($role))	{
			$this->db->where('LOWER(cizacl_role_name)',strtolower($role));
			$query = $this->db->get('cizacl_roles');
